...
export function Button(props: IButtonProps) {
  const { children, variant, className, arrowIcon, loading, ...otherProps } = props
  ...
  return (
    <button {...otherProps} className={twMerge(buttonClassName)}>
      ...
      <ArrowRightIcon className="absolute top-1/2 right-16 transform translate-y-[-50%] opacity-0 group-hover:opacity-100 group-hover:translate-x-2 transition-all duration-300 ease-in-out group-hover:text-black" />
    </button>
  )
}
