...
export async function respondToWeddingInvite(body: WeddingParams) {
  ...
  if (validateEmail) {
    return { message: 'Este email já foi utilizado para responder ao formulário.', data: validateEmail, status: 400 }
  }
  ...
  try {
    ...
    return { message: `Não foi possivel criar o usuario: ${(error as Error).message}`, status: 500 };
  }
